...
void simd_addition_multiply(float* a, float* b, float* result, float c, long long size) {
    __m256* xmm_a = (__m256*)a;
    __m256* xmm_b = (__m256*)b;
    __m256* xmm_result = (__m256*)result;
    __m256 xmm_c = _mm256_set1_ps(c);
    
    long long num_simd_ops = size / 8;

    for (long long i = 0; i < num_simd_ops; i++) {
        
        xmm_result[i] = _mm256_add_ps(_mm256_mul_ps(xmm_a[i], xmm_c), xmm_b[i]);
    }
...
int main() {
    ...
    std::ofstream outfile("Benchmark_Results/Saxpy_Cpp_SIMD_AVX2_CPU.csv");
